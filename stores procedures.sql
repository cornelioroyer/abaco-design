...
drop trigger t_rela_cglcomprobante1_cglposteo_delete on rela_cglcomprobante1_cglposteo;
drop trigger t_rela_cglcomprobante1_cglposteo_insert on rela_cglcomprobante1_cglposteo;
drop trigger t_rela_cgl_comprobante1_cglposteo_delete on rela_cgl_comprobante1_cglposteo;
drop trigger t_rela_cgl_comprobante1_cglposteo_insert on rela_cgl_comprobante1_cglposteo;



drop trigger t_cglposteo_insert on cglposteo;
drop trigger t_cglposteo_delete on cglposteo;
drop trigger t_cglposteo_update on cglposteo;


drop trigger t_cglposteoaux1_insert on cglposteoaux1;
drop trigger t_cglposteoaux1_delete on cglposteoaux1;
drop trigger t_cglposteoaux1_update on cglposteoaux1;


drop function f_cglcomprobante1_cglposteo(char(2), char(3), int4, int4, int4);
drop function f_cgl_comprobante1_cglposteo(char(2), int4);
drop function f_rela_cglcomprobante1_cglposteo_insert();
...
drop function f_cglsldocuenta_update_balance_inicio(char(2), char(24), int4, int4);
...
begin
    ...
    loop
        ...
        select into r_cglsldocuenta * from cglsldocuenta
                            where cglsldocuenta.cuenta = r_cglcuentas.cuenta
                            and cglsldocuenta.compania = as_compania
        ...
        while 1=1 loop
            ...
            select into r_cglsldocuenta2 * from cglsldocuenta
            where compania = as_compania
            and year = li_year
            and periodo = li_periodo
            and cuenta = r_cglcuentas.cuenta;
            if not found then
                insert into cglsldocuenta (compania, cuenta, year, periodo, balance_inicio,
                    debito, credito)
                values (as_compania, r_cglcuentas.cuenta, li_year, li_periodo, ldc_balance_final,
                    0, 0);
            else
                update cglsldocuenta
                set balance_inicio = ldc_balance_final
                ...
                ldc_balance_final := ldc_balance_final + r_cglsldocuenta2.debito - r_cglsldocuenta2.credito;
                
            end if;
            
        
            if li_year = Anio(current_date) and li_periodo >= Mes(current_date) then
                exit;
            end if;
        end loop;
        ...
        loop
            ...
            while 1=1 loop
                ...
                select into r_cglsldoaux12 * from cglsldoaux1
                where compania = as_compania
                and year = li_year
                and periodo = li_periodo
                ...
                if not found then
                    insert into cglsldoaux1 (compania, cuenta, auxiliar, year, periodo, balance_inicio,
                        debito, credito)
                    values (as_compania, r_cglcuentas.cuenta, r_cglsldoaux1.auxiliar, li_year, li_periodo, ldc_balance_final,
                        0, 0);
                else
                    update cglsldoaux1
                    set balance_inicio = ldc_balance_final
                    ...
                    ldc_balance_final := ldc_balance_final + r_cglsldoaux12.debito - r_cglsldoaux12.credito;
...
begin
    ...
    while 1=1 loop
        ...
        and periodo = li_periodo;
        if found then
            ldc_balance_inicio := ldc_balance_inicio + r_cglsldoaux1.balance_inicio + 
                                  r_cglsldoaux1.debito - r_cglsldoaux1.credito;
        end if;
...
begin
    ...
    li_consecutivo := f_cglposteoaux1(li_consecutivo, as_cuenta, as_aux1, adc_monto);
    
    li_consecutivo := f_cglposteoaux2(li_consecutivo, as_cuenta, as_aux1, adc_monto);
...
begin
    ...
    ldc_balance_final := r_cglsldoaux1.balance_inicio + adc_debito - adc_credito;
    while 1=1 loop
       li_periodo := li_periodo + 1;
       select into r_cglsldoaux1 * from cglsldoaux1
       where compania = as_compania
...
begin
    ...
    if found then
       ...
       values (ai_consecutivo, as_auxiliar, 0, ldc_debito, ldc_credito);
    end if;
...
create trigger t_rela_cglcomprobante1_cglposteo_insert after insert or update on rela_cglcomprobante1_cglposteo
for each row execute procedure f_rela_cglcomprobante1_cglposteo_insert();

create trigger t_rela_cgl_comprobante1_cglposteo_insert after insert or update on rela_cgl_comprobante1_cglposteo
for each row execute procedure f_rela_cgl_comprobante1_cglposteo_insert();

drop trigger t_rela_afi_cglposteo_delete on rela_afi_cglposteo;
drop function f_afi_cglposteo(char(2), int4, int4);
...
drop trigger t_bcotransac1_bcocircula_insert on bcotransac1;
drop trigger t_bcotransac1_bcocircula_delete on bcotransac1;
drop trigger t_bcotransac1_bcocircula_update on bcotransac1;

drop trigger t_bcocheck1_bcocircula_insert on bcocheck1;
drop trigger t_bcocheck1_bcocircula_delete on bcocheck1;
drop trigger t_bcocheck1_bcocircula_update on bcocheck1;
drop trigger t_rela_bcotransac1_cglposteo_delete on rela_bcotransac1_cglposteo;
drop trigger t_rela_bcocheck1_cglposteo_delete on rela_bcocheck1_cglposteo;
...
begin
    ...
    for r_work in select bcocheck2.cuenta, bcocheck2.auxiliar1, bcocheck2.auxiliar2, bcocheck2.monto
                    from bcocheck2
                    where bcocheck2.cod_ctabco = r_bcocheck1.cod_ctabco
                    and bcocheck2.no_cheque = r_bcocheck1.no_cheque
                    and bcocheck2.motivo_bco = r_bcocheck1.motivo_bco
                    order by 1
    loop
    ...
    for r_work in select proveedores.cuenta, sum(bcocheck3.monto) as monto
...
drop trigger t_rela_caja_trx1_cglposteo_delete on rela_caja_trx1_cglposteo;
drop trigger t_caja_trx1_update on caja_trx1;
drop trigger t_caja_trx2_update on caja_trx2;
...
drop trigger t_factura1_insert on factura1;
drop trigger t_factura1_update on factura1;
drop trigger t_factura1_delete on factura1;

drop trigger t_cxctrx1_delete on cxctrx1;
drop trigger t_cxctrx1_update on cxctrx1;
drop trigger t_cxctrx2_insert on cxctrx2;
drop trigger t_cxctrx2_update on cxctrx2;
drop trigger t_cxctrx2_delete on cxctrx2;
drop trigger t_rela_cxctrx1_cglposteo_delete on rela_cxctrx1_cglposteo;
drop trigger t_cxctrx3_delete on cxctrx3;
...
begin
    ...
    if r_cxctrx1.obs_ajuste_cxc is null then
       r_cxctrx1.obs_ajuste_cxc := ''TRANSACCION #  '' || r_cxctrx1.sec_ajuste_cxc;
    else
       r_cxctrx1.obs_ajuste_cxc := ''TRANSACCION #  '' || r_cxctrx1.sec_ajuste_cxc || ''  '' || trim(r_cxctrx1.obs_ajuste_cxc);
    end if;
...
begin
    ...
    if r_factura1.num_factura = 0 then
        r_factura1.num_factura = r_factura1.num_documento;
    end if;
...
*/    
    ...
    and docmto_aplicar = lc_docmto_aplicar;
    if not found then
    --        raise exception ''costo %'', r_factura1.cliente;

        insert into cxcdocm (documento, docmto_aplicar, cliente, motivo_cxc, almacen, 
        docmto_ref, motivo_ref, aplicacion_origen, uso_interno, fecha_docmto, 
        fecha_vmto, monto, fecha_posteo, status, usuario, fecha_captura, obs_docmto,
        fecha_cancelo, referencia) values (trim(lc_documento), trim(lc_docmto_aplicar), 
        r_factura1.cliente, r_factura1.tipo, r_factura1.almacen, lc_documento, 
        r_factura1.tipo, ''FAC'', ''N'', r_factura1.fecha_factura, 
...
drop trigger t_cxc_recibo1_delete on cxc_recibo1;
drop trigger t_cxc_recibo1_update on cxc_recibo1;

drop trigger t_cxc_recibo2_insert on cxc_recibo2;
drop trigger t_cxc_recibo2_update on cxc_recibo2;
drop trigger t_cxc_recibo2_delete on cxc_recibo2;

drop trigger t_rela_cxc_recibo1_cglposteo_delete on rela_cxc_recibo1_cglposteo;

drop function f_recibo_cxcdocm(char(2), int4);
           ...
           if not found then
              ...
              if found then
                 insert into cxcdocm (documento, docmto_aplicar, cliente, motivo_cxc, almacen, 
                    ...
                    fecha_vmto, monto, fecha_posteo, status, usuario, fecha_captura, obs_docmto,
                    fecha_cancelo, referencia) 
                 values (trim(r_cxc_recibo1.documento), trim(r_cxc_recibo2.documento_aplicar), 
                    r_cxc_recibo1.cliente, r_cxc_recibo1.motivo_cxc, r_cxc_recibo2.almacen_aplicar, 
                    trim(r_cxc_recibo2.documento_aplicar), r_cxc_recibo2.motivo_aplicar, ''CXC'', ''N'', 
...
drop trigger t_cxpfact1_delete on cxpfact1;
drop trigger t_cxpfact1_update on cxpfact1;
drop trigger t_cxpfact2_delete on cxpfact2;
drop trigger t_cxpfact2_update on cxpfact2;
drop trigger t_rela_cxpfact1_cglposteo_delete on rela_cxpfact1_cglposteo;
...
create trigger t_rela_cxpfact1_cglposteo_delete after delete or update on rela_cxpfact1_cglposteo
for each row execute procedure f_rela_cxpfact1_cglposteo_delete();

drop trigger t_rela_cxpajuste1_cglposteo_delete on rela_cxpajuste1_cglposteo;
drop trigger t_cxpajuste1_delete on cxpajuste1;
drop trigger t_cxpajuste1_update on cxpajuste1;

drop trigger t_cxpajuste2_delete on cxpajuste2;
drop trigger t_cxpajuste2_update on cxpajuste2;

drop trigger t_cxpajuste3_delete on cxpajuste3;
drop trigger t_cxpajuste3_update on cxpajuste3;
...
drop function f_cxpajuste1_cxpdocm(char(2), int4);
...
drop trigger t_rela_factura1_cglposteo_delete on rela_factura1_cglposteo;
drop function f_factura1_cglposteo(char(2), char(3), int4);
drop function f_rela_factura1_cglposteo_delete() cascade;
drop function f_monto_factura(char(2), char(3), int4);
...
begin
    ...
    ldc_work := ldc_monto_factura2 - ldc_sum_factura2 - ldc_sum_factura3;
    if ldc_work <> 0 then
       raise exception ''Factura % esta desbalanceada...Verifique %'',ai_num_documento, ldc_work ;
    end if;
    ...
    li_consecutivo := f_cglposteo(r_almacen.compania, ''FAC'', r_factura1.fecha_factura, 
                            ls_cuenta, null, null, r_factmotivos.tipo_comp, 
    ...
    loop
        li_consecutivo := f_cglposteo(r_almacen.compania, ''FAC'', r_factura1.fecha_factura, 
                                r_work.cuenta, null, null, r_factmotivos.tipo_comp, 
                                trim(r_factura1.observacion), r_work.monto);
        if li_consecutivo > 0 then
            insert into rela_factura1_cglposteo (almacen, tipo, num_documento, consecutivo)
            values (r_factura1.almacen, r_factura1.tipo, r_factura1.num_documento, li_consecutivo);
...
drop trigger t_rela_eys1_cglposteo_delete on rela_eys1_cglposteo;
drop trigger t_factura2_eys2_delete on factura2_eys2;
drop trigger t_eys4_delete on eys4;
drop trigger t_cos_consumo_eys2_delete on cos_consumo_eys2;
drop trigger t_cos_produccion_eys2_delete on cos_produccion_eys2;
drop trigger t_eys2_eys6 on eys2;
drop trigger t_eys2_insert on eys2;
drop trigger t_eys2_delete on eys2;
drop trigger t_eys2_update on eys2;


drop function f_eys1_cglposteo(char(2), int4);
drop function f_rela_eys1_cglposteo_delete() cascade;
drop function f_factura2_eys2_delete();
drop function f_eys4_delete();
drop function f_cos_consumo_eys2_delete();
drop function f_cos_produccion_eys2_delete();


drop function f_eys2_eys6();
drop function f_costos_inventario(char(2), int4);
drop function f_costos_inventario_fifo(char(2), int4);
drop function f_eys1_poner_cuentas(char(2), int4);
drop function f_eys2_insert() cascade;
drop function f_eys2_delete() cascade;
drop function f_eys2_update() cascade;
drop function f_factura_inventario(char(2), char(3), integer);
drop function f_update_existencias(char(2), char(15));
drop function f_compras_inventario(char(2), char(6), char(25));
...
begin
    ...
    for r_work in select almacen.compania, eys3.cuenta, sum(eys3.monto) as monto
                    from eys3, almacen
...
begin
    ...
    loop
        if r_factmotivos.devolucion = ''S'' then
            select into ldc_costo, ldc_cantidad eys2.costo, eys2.cantidad
...
--        raise exception ''costo %'', ldc_cu;
        ...
        if ls_cuenta_costo is null then
            raise exception ''Para la factura %'', r_factura2.num_documento;
        end if;
...
create trigger t_eys2_insert after update on eys2
...
drop trigger t_nomtpla2_update on nomtpla2;
drop trigger t_rela_nomctrac_cglposteo_delete on rela_nomctrac_cglposteo;

drop function f_nomtpla2_update() cascade;
drop function f_rela_nomctrac_cglposteo_delete() cascade;
drop function f_nomctrac_cglposteo(char(2), char(7), char(2), char(2), int4, int4);
