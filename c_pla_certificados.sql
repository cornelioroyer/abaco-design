...
drop index i1_pla_certificados cascade;

drop index i2_pla_certificados cascade;

drop table pla_certificados cascade;

create table pla_certificados (
compania             char(2)              not null,
codigo_empleado      char(7)              not null,
f_desde              date                 not null,
h_desde              time                 not null,
...
alter table pla_certificados
   add constraint pk_pla_certificados primary key (codigo_empleado, compania, f_desde, h_desde);

create unique index i1_pla_certificados on pla_certificados (
codigo_empleado,
compania,
f_desde,
h_desde
...
create  index i2_pla_certificados on pla_certificados (
codigo_empleado,
compania
);

alter table pla_certificados
   add constraint fk_pla_certificado_ref_rhuempl foreign key (codigo_empleado, compania)
      references rhuempl (codigo_empleado, compania)
