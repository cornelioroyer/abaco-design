...
--        r_adc_parametros_contables.cta_costo := ''8108'';
...
begin
    ...
    if found then
        ...
        values (r_adc_house.almacen, r_factmotivos.tipo, li_num_documento,
            r_adc_house.cliente, r_clientes.forma_pago, r_adc_manifiesto.vendedor, r_clientes.nomb_cliente, 
...
begin
    ...
    if not found then
        ...
        and factura1.no_referencia = r_adc_manifiesto.no_referencia
        and factura1.mbl =r_adc_master.no_bill
        and factura1.hbl = r_adc_house.no_house
        and factura1.vapor = r_adc_manifiesto.vapor;
        if found then
            li_num_documento := r_factura1.num_documento;            
        else
            li_num_documento := 0;
        ...
        values (r_adc_manejo.almacen, r_factmotivos.tipo, li_num_documento, li_linea,
            r_adc_manejo.articulo, 1, r_adc_manejo.cargo, 0, 0, ls_work, 0);
        ...
        if found then
            ...
            insert into factura4 (almacen, tipo, num_documento, rubro_fact_cxc, monto)
            values (r_adc_manejo.almacen, r_factmotivos.tipo, li_num_documento, ''SUB-TOTAL'', ldc_cargo);
        end if;
    end if;            
    return 1;
end;
...
drop function f_cglposteo_before_delete_old() cascade;
...
begin
    ...
    loop
        i := f_cxc_recibo1_cglposteo(r_cxc_recibo1.almacen, r_cxc_recibo1.consecutivo);
    end loop;        
    ...
    for r_cxctrx1 in select cxctrx1.* from almacen, cxctrx1
                    ...
                    and not exists
                        ...
                        where rela_cxctrx1_cglposteo.almacen = cxctrx1.almacen
                        and rela_cxctrx1_cglposteo.sec_ajuste_cxc = cxctrx1.sec_ajuste_cxc)
                    order by cxctrx1.fecha_posteo_ajuste_cxc
    loop
        i := f_cxctrx1_cglposteo(r_cxctrx1.almacen, r_cxctrx1.sec_ajuste_cxc);
    end loop;        
...
begin
	...
	select documento, docmto_aplicar, cliente, motivo_cxc, almacen, docmto_ref,
	...
	and fecha_posteo >= ld_fecha
	and almacen in (select almacen from almacen where compania = as_cia)
	and not exists
	(select * from cxcdocm
...
drop function f_cxc_recibo1_after_update() cascade;
drop function f_cxc_recibo2_before_delete() cascade;
drop function f_cxc_recibo2_before_update() cascade;
...
create function f_cxc_recibo1_after_update() returns trigger as '
declare
    li_work integer;
begin
    delete from rela_cxc_recibo1_cglposteo
...
create trigger t_cxc_recibo1_after_update after update on cxc_recibo1
for each row execute procedure f_cxc_recibo1_after_update();
...
drop function f_factura2_eys2_after_update() cascade;
drop function f_factura2_eys2_after_delete() cascade;

drop function f_postea_fac(char(2)) cascade;
drop function f_factura2_eys2(char(2), char(3), int4, int4) cascade;

drop function f_factura4_before_update() cascade;
drop function f_factura4_before_delete() cascade;


create function f_factura2_eys2(char(2), char(3), int4, int4) returns integer as '
declare
    as_almacen alias for $1;
    as_tipo alias for $2;
    ai_num_documento alias for $3;
...
begin
    ...
    for r_factura1 in select factura1.* from almacen, factura1
    ...
    loop
        i := f_factura1_cglposteo(r_factura1.almacen, r_factura1.tipo, r_factura1.num_documento);
    end loop;        
...
create function f_factura2_eys2_after_delete() returns trigger as '
begin
    delete from eys2
    where almacen = old.almacen
...
create function f_factura2_eys2_after_update() returns trigger as '
begin
    delete from eys2
    where almacen = old.almacen
    and no_transaccion = old.no_transaccion
    ...
    loop
        ...
        if found then
            ...
            if ls_cuenta is null or not found then
                ...
                and articulo = r_work.articulo;
                if found then
                    select into ls_cuenta fac_parametros_contables.vtas_exentas from fac_parametros_contables, articulos_agrupados
                    where articulos_agrupados.codigo_valor_grupo = fac_parametros_contables.codigo_valor_grupo
...
declare
    i integer;
begin
    i = f_factura2_eys2(new.almacen, new.tipo, new.num_documento, new.linea);
    return new;
end;
' language plpgsql;
...
begin
    ...
    if old.almacen <> new.almacen
        ...
        or old.cliente <> new.cliente then
        delete from cxcdocm
        where almacen = old.almacen
        and cliente = old.cliente
        and motivo_cxc = old.tipo
...
create trigger t_factura2_eys2_after_update after update on factura2_eys2
for each row execute procedure f_factura2_eys2_after_update();

create trigger t_factura2_eys2_after_delete after delete on factura2_eys2
for each row execute procedure f_factura2_eys2_after_delete();
...
begin
    ...
    for r_cos_trx in select cos_trx.* from cos_trx
                        ...
                        and not exists
                            ...
                            where cos_produccion_eys2.compania = cos_trx.compania
                            and cos_produccion_eys2.secuencia = cos_trx.secuencia)
                        and not exists
                            (select * from cos_consumo_eys2
                            where cos_consumo_eys2.compania = cos_trx.compania
                            and cos_consumo_eys2.secuencia = cos_trx.secuencia)
                        order by fecha
    loop
        select into i f_costos_inventario(r_cos_trx.compania, r_cos_trx.secuencia);
...
*/
    ...
    loop
        i = f_factura2_eys2(r_factura2.almacen, r_factura2.tipo, r_factura2.num_documento, r_factura2.linea);
...
declare
    i integer;
begin
    i := f_valida_fecha(old.compania, ''CGL'', old.fecha_laborable);
    return old;
end;
' language plpgsql;
...
*/            
    ...
    select into r_clientes * from clientes, cglcuentas
    where clientes.cuenta = cglcuentas.cuenta
    and cglcuentas.auxiliar_1 = ''S''
    and clientes.cliente = r_tal_ot1.cliente;
    if found then
